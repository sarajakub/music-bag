...
void loop() {
  ...
  if (touchValue >= 500 && touchValue <= 600) {
    noteFrequency = C4;  // A note (440 Hz)
  } 
  else if (touchValue >= 601 && touchValue <= 700) {
    noteFrequency = D4;  // B note (466.16 Hz)
  } 
  else if (touchValue >= 701 && touchValue <= maxTouchValue) {
    noteFrequency = E4;  // C note (523.25 Hz)
  }
